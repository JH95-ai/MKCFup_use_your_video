	...
	const Mat &k_hog, const Mat &k_cn, const complex_mat &yf, int frame, int start_frame, float learning_rate_cn, float learning_rate_hog, const Mat &y, int imchannel, float lambda)
{
	...
	Mat temp1, temp2, temp, out_temp, prevalpha, deltaalpha;
	...
	while (stop == 0)
	{
		...
		Mat tmp = 2 * y;
		Mat alpha(tmp.rows, tmp.cols, CV_32FC1);
...
int main()
{
	...
	//cv::VideoCapture capture(0);
        cv::VideoCapture capture("  your_video");
	if (!capture.isOpened())
	{
	...
	capture >> tmpMat;
	cv::resize(tmpMat, tmpMat, cv::Size(640, 360));
        //your detector models init init_gt;
	...
	//cv::Mat test(use_sz[0], use_sz[1], CV_32FC1, y);
	Mat kk, kk2, temp;
	vector<Mat> channels(2);
	complex_mat yf = newfft2(y);
	Mat cos_window = cosine_window_function(use_sz[1], use_sz[0]);

	int imchannel = tmpMat.channels();
	if (tmpMat.empty()) cout << "Can't load image！" << endl;
	...
	if (use_dsst)
	{
		...
		max_scale_factor = pow(scale_step, floor(log(min(float(tmpMat.rows) / float(base_target_sz[0]), float(tmpMat.cols) / float(base_target_sz[1]))) / log(scale_step)));
	}
	//model initialize
	...
	while (1)
	{
		Mat track_frame;
		capture >> track_frame;
		cv::resize(track_frame, track_frame, cv::Size(640, 360));
		...
		if (frame>start_frame)
		{
			Mat patch = get_subwindow(track_frame, pos[1], pos[0], sz[1], sz[0], currentScaleFactor);
			...
			//target location
			Point2i max_loc;
			cv::minMaxLoc(response, 0, 0, 0, &max_loc);
			...
			if (use_dsst)
			{
				Mat xs_pca = get_scale_subwindow(track_frame, pos[1], pos[0], base_target_sz[1], base_target_sz[0], scaleSizeFactors, scale_model_sz, currentScaleFactor, nScales, featureRatio, num_hog_fea);
				Mat temp_den = xs_pca;
				temp_den = temp_den.mul(mega_scale_window);

				dft(temp_den, kk, DFT_COMPLEX_OUTPUT);
				cv::split(kk, channels);
				...
				cv::reduce(temp_sf_test.real, temp_scale_responsef_test.real, 0, CV_REDUCE_SUM, CV_32FC1);
				cv::reduce(temp_sf_test.img, temp_scale_responsef_test.img, 0, CV_REDUCE_SUM, CV_32FC1);
				...
				merge(channels, kk);
				cv::idft(kk, kk2);
				cv::split(kk2, channels);
				Mat interp_scale_response = channels[0].clone();
				cv::minMaxLoc(interp_scale_response, 0, 0, 0, &max_loc);
		...
		if (use_dsst)
		{
			...
			dft(temp, kk, DFT_COMPLEX_OUTPUT);
			...
			cv::reduce(temp_sf_test.real, new_sf_den_test.real, 0, CV_REDUCE_SUM, CV_32FC1);
			cv::reduce(temp_sf_test.img, new_sf_den_test.img, 0, CV_REDUCE_SUM, CV_32FC1);
			if (frame == start_frame)
			{
		...
		if (visualization)
		{
			rectangle(track_frame, cvPoint(pos[1] - target_sz[1] / 2, pos[0] - target_sz[0] / 2), cvPoint(pos[1] + target_sz[1] / 2, pos[0] + target_sz[0] / 2), Scalar(255, 0, 0), 1, 1, 0);
			//sprintf(buf, "Seq: %s", name);
			namedWindow("result");
			imshow("result", track_frame);
			waitKey(1);
		}
		}
